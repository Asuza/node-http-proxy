...
ProxyServer.prototype.listen = function(port, hostname) {
  ...
  closure = function(req, res) {
    var route = self.getRouteData(req);

    if (route.newPath) {
      req.url = route.newPath;
    }
...
ProxyServer.prototype.getRouteData = function(request) {
  var routes = this.options.router,
      data = {},
      prefixLength,
      newPath,
      target,
  ...
  if (routes) {
    for (route in routes) {
      if (request.url.indexOf(route) === 0) {
        ...
        newPath = request.url.substring(prefixLength);
        break;
      }
    }
  }
  ...
  if (target) {
    data.target = target;
    data.oldPath = request.url;
    data.newPath = newPath;

    console.log(data.oldPath, '=>', target + newPath);
  } else {
    data.target = routes['*'];
  }
